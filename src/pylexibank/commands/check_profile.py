...
def check_profile(dataset, args):
    ...
    for row in dataset.cldf_dir.read_csv('forms.csv', dicts=True):
        tokens = dataset.tokenizer(row, row['Form'], column='IPA') if \
    ...
    if generated:
        ...
        with Table(
            ...
            for tk, values in sorted(
                    generated.items(), key=lambda x: len(x[1])):
                table.append([
                    tk,
    ...
    if modified:
        ...
        with Table(
            ...
            for tk, values in sorted(
                    modified.items(), key=lambda x: len(x[1])):
                table.append([
                    tk,
                    str(args.clts.api.bipa[tk]),
                    values[0][0],
    ...
    if unknown:
        ...
        with Table(
            ...
            for tk, values in sorted(
                    unknown.items(), key=lambda x: len(x[1])):
                table.append([
                    tk,
                    "◌"+tk,
    ...
    if missing:
        ...
        with Table(
            ...
            for tk, values in sorted(
                    missing.items(), key=lambda x: len(x[1])):
                table.append([
                    tk,
                    "◌"+tk,
                    args.clts.api.bipa[tk].codepoints,
